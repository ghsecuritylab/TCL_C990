					 ...
					 struct wpa_scan_res *bss)
{
	...
	if (proto_match == 0)
#ifdef WAPI
		wpa_printf(MSG_DEBUG, "   skip - no WPA/RSN/WAPI proto match");
#else
...
#ifdef WAPI
		if (selected->wapi_ie_len == 0) {
			/* only do this when associating to a non-wapi AP: can this be a problem? */
#endif
