...
#define CONFIG_MSM_RMNET_WAKELOCK_TIME		2
static int msm_rmnet_wakelock_time = CONFIG_MSM_RMNET_WAKELOCK_TIME;
module_param_named(wakelock_time, msm_rmnet_wakelock_time,
                        int, S_IRUGO | S_IWUSR | S_IWGRP);
	...
	for (;;) {
		...
		if (RMNET_IS_MODE_IP(opmode) ? (sz > dev->mtu) :
			...
			if (skb == NULL) {
				...
				ptr = skb_put(skb, sz);
				wake_lock_timeout(&p->wake_lock, msm_rmnet_wakelock_time * HZ);
