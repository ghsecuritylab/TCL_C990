...
static struct flash_identification ununiform_flash[] =
{
	/* Flash ID density(MB) Wid Pgsz  Blksz     oobsz   ECC      Manuf */
	{0x1190bcad, (512<<20), 1,  4096, (4096<<6), 128,    1, },
};
...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	if (!flash_onfi_probe(chip)) {
		dev_found = 1;
	} else { /* search the devices as un-uniform device id at first */
		flash_id = flash_read_id(chip);
		for(index = 0; index < ARRAY_SIZE(ununiform_flash); ++index)
			if(flash_id == ununiform_flash[index].flash_id) {
				dev_found = 1;
				supported_flash.flash_id  = ununiform_flash[index].flash_id;
				supported_flash.density  = ununiform_flash[index].density;
				supported_flash.widebus  = ununiform_flash[index].widebus;
				supported_flash.pagesize  = ununiform_flash[index].pagesize;
				supported_flash.blksize  = ununiform_flash[index].blksize;
				supported_flash.oobsize	= ununiform_flash[index].oobsize;
				supported_flash.ecc_correctability =
				ununiform_flash[index].ecc_correctability;
				break;
			}
