...
#if defined(TOUCHSCREEN_FT5306)
#define FT5X0X_POWER_ONOFF(pdata, onoff) do {} while(0)
#else
#define FT5X0X_POWER_ONOFF(pdata, onoff) \
	do { \
...
#if defined(CONFIG_SUPPORT_FTS_CTP_UPG) || !defined(CONFIG_PROJECT_QRD)
...
//	event->touch_point = buf[2] & 0x03;// 0000 0011
	...
	dev_dbg(&this_client->dev, "%s: 1:%d %d 2:%d %d \n", __func__,
		event->au16_x[0], event->au16_y[0], event->au16_x[1], event->au16_y[1]);
...
{
	int ret = -1;
	struct ft5x0x_ts_data *ft5x0x_ts = container_of(work, struct ft5x0x_ts_data,
					      pen_event_work);
...
static void ft5x0x_ts_resume(struct early_suspend *handler)
{
	...
	FT5X0X_POWER_ONOFF(ts->pdata, 1);
	printk("%s:%d (%d, %d)\n", __func__, ts->event.touch_point, ts->event.au16_x[0], ts->event.au16_y[0]);
#if defined(TOUCHSCREEN_FT5206)
	// wake the mode
	if (ts->pdata->reset >= 0) {
	...
	gpio_set_value(ts->pdata->wake, 1);
	usleep(1);
	gpio_set_value(ts->pdata->wake, 0);
	msleep(10);
	gpio_set_value(ts->pdata->wake, 1);
#endif	
...
//#ifdef CONFIG_SUPPORT_FTS_CTP_UPG
	...
	if (pdata == NULL || pdata->irq == 0) {
		dev_err(&client->dev, "%s: platform data is null or not enough\n", __func__);
		goto exit_platform_data_null;
	}

	FT5X0X_POWER_ONOFF(pdata, 1);
	...
	gpio_set_value(pdata->wake, 1);
	usleep(1);
	gpio_set_value(pdata->wake, 0);
	msleep(10);
	gpio_set_value(pdata->wake, 1);
#endif
	...
	msleep(300); // wait Min 300ms for the TP work correctly so that we can read hardware version
	uc_reg_value = ft5x0x_read_fw_ver();
	printk("[FST] Firmware version = 0x%x\n", uc_reg_value);
	if (uc_reg_value < 0) {
		printk("[FST]ft5x0x Detect Faild!!\n");
		goto exit_devices_detect;
	}
...
static int __devexit ft5x0x_ts_remove(struct i2c_client *client)
{
	...
	destroy_workqueue(ft5x0x_ts->ts_workqueue);
	i2c_set_clientdata(client, NULL);
	if (ft5x0x_ts->pdata->reset >= 0)
		gpio_free(ft5x0x_ts->pdata->reset);
	FT5X0X_POWER_ONOFF(ft5x0x_ts->pdata, 0);
	kfree(ft5x0x_ts);
