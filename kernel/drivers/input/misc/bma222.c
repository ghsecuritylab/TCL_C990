...
#else
	input_report_rel(bma222->input, REL_RX, acc.x * 4);
	input_report_rel(bma222->input, REL_RY, acc.y * 4);
	input_report_rel(bma222->input, REL_RZ, acc.z * 4);
#endif

#if DEBUG
	pr_info("%s, acc.x:%d, acc.y:%d, acc.z:%d\n", __func__, acc.x, acc.y, acc.z);
#endif
		...
		const char *buf, size_t count)
{
	...
	pr_info("%s, delay:%d", __func__, (int)data);
...
static void bma222_set_enable(struct device *dev, int enable)
{
	...
	pr_info("%s, enable:%d, pre_enable:%d\n", __func__, enable, pre_enable);
