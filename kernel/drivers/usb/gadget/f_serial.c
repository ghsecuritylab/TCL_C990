...
#ifdef CONFIG_USB_F_SERIAL
 #ifdef CONFIG_PCUI_SUPPORT
int fserial_pcui_bind_config(struct usb_configuration *c)
{
	return gser_bind_config(c, 2);
}
static struct android_usb_function pcui_function = {
	.name = "pcui",
	.bind_config = fserial_pcui_bind_config,
};
#endif
...
static int __init fserial_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i <pdata->no_ports /*GSERIAL_NO_PORTS*/; i++) {
...
probe_android_register:
 ...
 #ifdef CONFIG_PCUI_SUPPORT
	android_register_function(&pcui_function);
 #endif
	return 0;
}
