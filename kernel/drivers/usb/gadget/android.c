...
int android_enable_function(struct usb_function *f, int enable)
{
	...
	if(dev->factory_mode){
		pr_info_ratelimited("ANDROID USB in factory_mode\n");
		return 0;
	}
...
#endif
	{
		unsigned int factory_mode = 0;//tian
		extern int msm_oem_read_nv(unsigned int nv_item, void *buf);
		msm_oem_read_nv(7236, &factory_mode);
		pr_info_ratelimited("ANDROID USB in factory_mode =%x\n", factory_mode);
		if(0x12345678 == factory_mode){
			dev->factory_mode = true;
		}else
			dev->factory_mode = false;
	}
