...
#define	BATTERY_RPC_VER_1_1		0x00050001
...
#if DEBUG
#define DBG_LIMIT(x...) do {printk(KERN_INFO x); } while (0)
#else
...
void msm_battery_update_psy_status(void)
{
	...
	DBG_LIMIT("BATT: received, %d, %d, 0x%x; %d, %d, %d, %d; %d, %d, %d\n",
	...
	if (msm_battery_info.charger_status != charger_status) {
		if (msm_battery_info.charger_status == CHARGER_STATUS_NULL) {
			DBG_LIMIT("BATT: start charging\n");

			if (charger_hardware == CHARGER_TYPE_USB_PC) {
				DBG_LIMIT("BATT: usb pc charger inserted\n");
				...
				DBG_LIMIT("BATT: usb wall changer inserted\n");
				...
				DBG_LIMIT("BATT: unknown changer inserted\n");

				msm_battery_info.current_psy = &msm_psy_ac;
				msm_battery_info.current_charger_src = AC_CHG;
			} else {
				DBG_LIMIT("BATT: CAUTION: charger hardware\n");

				msm_battery_info.current_psy = &msm_psy_ac;
				msm_battery_info.current_charger_src = AC_CHG;
			}
		} else if (charger_status == CHARGER_STATUS_NULL) {
			DBG_LIMIT("BATT: end charging\n");

			if (msm_battery_info.current_charger_src & USB_CHG) {
				DBG_LIMIT("BATT: usb pc charger removed\n");
			} else if (msm_battery_info.current_charger_src & AC_CHG) {
				DBG_LIMIT("BATT: usb wall charger removed\n");
			} else {
				DBG_LIMIT("BATT: CAUTION: charger invalid: %d\n",
	...
	if (msm_battery_info.battery_status != battery_status) {
		if (battery_status == BATTERY_STATUS_NULL) {
			DBG_LIMIT("BATT: battery removed\n");
...
#ifndef CONFIG_BATTERY_MSM_FAKE
	rc = msm_battery_register(BATTERY_LOW,//msm_battery_info.voltage_min_design,
...
static int __devinit msm_battery_init_rpc(void)
{
	...
	DBG_LIMIT("BATT: CAUTION: fake msm battery\n");
#else
	...
	msm_battery_info.battery_client = msm_rpc_register_client(
			"battery", BATTERY_RPC_PROG, BATTERY_RPC_VER_1_1, 1,
			msm_battery_cb_func);
	if (msm_battery_info.battery_client == NULL) {
		pr_err("BATT: %s, ERROR: rpc battery no client\n", __func__);
		return -ENODEV;
	}
	if (IS_ERR(msm_battery_info.battery_client)) {
		rc = PTR_ERR(msm_battery_info.battery_client);
		pr_err("BATT: %s, ERROR: rpc battery not register, rc=%d\n",
		       __func__, rc);
		msm_battery_info.battery_client = NULL;
		return -ENODEV;
	}
	msm_battery_info.battery_api_version = BATTERY_RPC_VER_1_1;
#endif /* CONFIG_BATTERY_MSM_FAKE */

	DBG_LIMIT("BATT: %s, rpc version charger=0x%08x, battery=0x%08x\n",
