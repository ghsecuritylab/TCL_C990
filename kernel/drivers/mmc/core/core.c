 ...
 */
int __mmc_claim_host_rpm(struct mmc_host *host, atomic_t *abort, int rpm_enabled)
{
	...
	while (rpm_enabled && mmc_dev(host)->power.runtime_status == RPM_SUSPENDING) {
		if (host->suspend_task == current)
			break;
                       count++;
                       if(!count % 100) {
                               printk("%s: suspendtask pid:%u tgid:%u, current's pid %u tgid %u\n", __func__, host->suspend_task->pid, host->suspend_task->tgid, current->pid, current->tgid);
                               spin_lock_irqsave(&host->lock, flags);
                               if(host->claimed) {
                                       printk("%s: is claimed and claimer is pid:%u tgid:%u\n", __func__, host->claimer->pid, host->claimer->tgid);
                               }
...
{
       return __mmc_claim_host_rpm(host, abort, 1);
}
...
void mmc_rescan(struct work_struct *work)
{
	 ...
	 *avoid deadlock happend on runtime suspend when all want to wait for the work to finish but the work need claim host
         *when cannot be done be in the SUSPENDING state.
         */
