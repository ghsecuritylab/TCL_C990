	...
	if (dev->power.runtime_status == RPM_SUSPENDING) {
		//if (mmc->suspend_task == current) {
               if (mmc->suspend_task != NULL) {
                       pr_info("%s: %s: RPM_SUSPENDING------- mmc->suspend_task:%d\n", mmc_hostname(mmc),
                               __func__, mmc->suspend_task->pid);
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	if (mmc) {
		...
		if (unlikely(work_busy(&mmc->detect.work)))
			rc = -EAGAIN;
		else if (mmc->card && (mmc->card->type != MMC_TYPE_SDIO)) {	//modified by zq on 120515, for wifi sleep bug.
			rc = mmc_suspend_host(mmc);
			printk("host->pdev_id:%d\n", host->pdev_id);
		}	
		pm_runtime_put_noidle(dev);

		if (!rc) {
/*			if (mmc->card && (mmc->card->type == MMC_TYPE_SDIO) &&		//modified by zq on 120515, for wifi sleep bug.
				(mmc->pm_flags & MMC_PM_WAKE_SDIO_IRQ)) {
				disable_irq(host->core_irqres->start);
				host->sdcc_irq_disabled = 1;
*/
				...
				mmc->ios.clock = 0;
				mmc->ops->set_ios(host->mmc, &host->mmc->ios);
/*				enable_irq(host->core_irqres->start);		//modified by zq on 120515, for wifi sleep bug.
				host->sdcc_irq_disabled = 0;
