...
static int mipi_dsi_probe(struct platform_device *pdev)
{
	...
	if (mfd->index == 0)
		...
		mfd->fb_imgType = MDP_RGBA_8888; // Tony modify on 20120121, must set to 32 bit, otherwise HAL will use RGB565 format
...
int mipi_dsi_driver_late_init(void)
{
	int ret;
...
EXPORT_SYMBOL(mipi_dsi_driver_late_init);
