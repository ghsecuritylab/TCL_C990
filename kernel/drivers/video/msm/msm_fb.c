...
/* Tony modify on 20120324 */
//#define INIT_IMAGE_FILE_PREFIX_CU "/initlogo.rle_cu"
//#define INIT_IMAGE_FILE_PREFIX "/initlogo.rle"
...
/* Tony add for getting boot logo format passed from LK, 20120324 */
#ifdef CONFIG_FB_MSM_LOGO
static int boot_logo_id = 0;
static int __init boot_logo_setup(char *bootlogo)
{
	if(!strcmp(bootlogo, "cu"))
		boot_logo_id = 1;
	return 1;
}
__setup("androidboot.logo=", boot_logo_setup);
#endif
/* Tony add end */
...
static int msm_fb_probe(struct platform_device *pdev)
{
	...
	if (!boot_splash) {
		...
		printk("%s: register lcd-backlight\n", __func__);
...
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
	...
	int *id;
	int fbram_offset;
	/* Tony add on 20120324 */	
#ifdef INIT_IMAGE_FILE_PREFIX
...
#ifdef INIT_IMAGE_FILE_PREFIX
	if(boot_logo_id == 1) {
		sprintf(fname, "%s_%dx%d", INIT_IMAGE_FILE_PREFIX_CU,
				panel_info->xres, panel_info->yres);
	} else {
		sprintf(fname, "%s_%dx%d", INIT_IMAGE_FILE_PREFIX,
				panel_info->xres, panel_info->yres);
	}
	if (!load_565rle_image(fname)) ;	/* Flip buffer */
#else // INIT_IMAGE_FILE_PREFIX
	if (!load_565rle_image(INIT_IMAGE_FILE)) ;	/* Flip buffer */
#endif // INIT_IMAGE_FILE_PREFIX
	/* Tony modify end */
#endif
	ret = 0;
...
EXPORT_SYMBOL(msm_fb_late_init);

int __init msm_fb_init(void)
{
  if (boot_splash) {
    /* do nothing for boot splash */
  } else {
    msm_fb_late_init();
  }
  return 0;
