...
msmrtc_timeremote_set_alarm(struct msm_rtc *rtc_pdata, unsigned long expiration)
{
	...
	if (rc)
		printk(KERN_EMERG "set alarm in mp error %d \n", rc);
...
static int msmrtc_cb_func(struct msm_rpc_client *client, void *buffer, int size)
{
	...
	msm_rpc_start_accepted_reply(client, recv->xid, RPC_ACCEPTSTAT_SUCCESS);
...
static int __devinit msmrtc_probe(struct platform_device *pdev)
{
	int rc;
	...
	else if (pdev->id == (TIMEREMOTE_PROG_VER_2 & RPC_VERSION_MAJOR_MASK))
...
static void
msmrtc_alarmtimer_expired(unsigned long _data, struct msm_rtc *rtc_pdata)
{
...
static int msmrtc_suspend(struct platform_device *dev, pm_message_t state)
{
	int rc, diff;
	struct rtc_time tm;
	unsigned long now;
