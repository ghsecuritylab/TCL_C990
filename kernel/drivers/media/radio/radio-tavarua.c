...
/* this macro switches the support for FM.apk and FMPlayer.apk */
#undef FM_ANDROID_APP_SUPPORT
...
static int xfr_intf_own(struct tavarua_device *radio)
{
	...
	if (radio->xfr_in_progress) {
		...
		/* FMPlayer.apk does not use interruption,
		so add complete(&radio->sync_req_done) here. */
...
static int tavarua_search(struct tavarua_device *radio, int on, int dir)
{
	...
	if (on) {
		...
		/* Loop till seek operation has completed */
		timeout = jiffies + msecs_to_jiffies(seek_timeout);
		do {
//			msleep(20);
			retval = tavarua_read_registers(radio, STATUS_REG1, 1);
			if (retval < 0)
				goto stop;
			timed_out = time_after(jiffies, timeout);
		}while (((radio->registers[STATUS_REG1] & SEARCH) == 0) && (!timed_out));
		if (radio->registers[STATUS_REG1] & SEARCH)
			FMDBG("Seek completes\n");
		if (timed_out)
			FMDBG("Seek timed out after %u ms\n", seek_timeout);
	} else {
		radio->registers[SRCHCTRL] &= ~SRCH_ON;
		radio->srch_params.get_list = 0;
		radio->registers[SRCHCTRL] = (dir << 3) |
...
stop:
	/* stop seeking */
	radio->registers[SRCHCTRL] &= ~SRCH_ON;
	retval = tavarua_write_register(radio, SRCHCTRL, radio->registers[SRCHCTRL]);
...
static int tavarua_fops_open(struct file *file)
{
	...
	for (i = 0; i < 10; i++)  {
		msleep(100);
		tavarua_read_registers(radio, STATUS_REG1, 1);
		if (radio->registers[STATUS_REG1] & READY)
			break;
		else
			pr_err("%s: FM open loop %d.", __func__, i);
	}
#endif
	/* get Chip ID */
		...
		if (!wait_for_completion_timeout(&radio->sync_req_done,
			...
			if (retval > 0) {
				if (getFreq.frequency == freq->frequency) {
					/** This is success, queue the event*/
					tavarua_q_event(radio,
