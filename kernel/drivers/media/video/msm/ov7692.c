...
static int ov7692_i2c_rxdata(unsigned short saddr, unsigned char *rxdata, int length)
{
    struct i2c_msg msgs[] = {
        {
            .addr  = saddr,
...
static int32_t ov7692_i2c_txdata(unsigned short saddr, unsigned char *txdata, int length)
{
    struct i2c_msg msg[] = {
        {
            .addr = saddr,
...
static int32_t ov7692_i2c_read(uint8_t raddr, uint8_t *rdata, int rlen)
{
    int32_t rc = 0;
    unsigned char buf[1];
    if (!rdata)
...
static int32_t OV7692_WritePRegs(POV7692_WREG pTb, int32_t len)
{
    int32_t i, ret = 0;
    uint8_t regv;
    for (i = 0; i < len; i++)
...
static int ov7692_set_exposure_compensation(int compensation)
{
	...
	switch(compensation)
	{
		case CAMERA_EXPOSURE_COMPENSATION_LV0:
			...
			rc = OV7692Core_WritePREG(ov7692_exposure_compensation_lv0_tbl);
			break;
		case CAMERA_EXPOSURE_COMPENSATION_LV1:
			CDBG("--CAMERA--CAMERA_EXPOSURE_COMPENSATION_LV1\n");
			rc = OV7692Core_WritePREG(ov7692_exposure_compensation_lv1_tbl);
			break;
		case CAMERA_EXPOSURE_COMPENSATION_LV2:
			CDBG("--CAMERA--CAMERA_EXPOSURE_COMPENSATION_LV2\n");
			rc = OV7692Core_WritePREG(ov7692_exposure_compensation_lv2_default_tbl);
			break;
		case CAMERA_EXPOSURE_COMPENSATION_LV3:
			CDBG("--CAMERA--CAMERA_EXPOSURE_COMPENSATION_LV3\n");
			rc = OV7692Core_WritePREG(ov7692_exposure_compensation_lv3_tbl);
			break;
		case CAMERA_EXPOSURE_COMPENSATION_LV4:
			CDBG("--CAMERA--CAMERA_EXPOSURE_COMPENSATION_LV3\n");
			rc = OV7692Core_WritePREG(ov7692_exposure_compensation_lv4_tbl);
			break;
		default:
	...
	if (effect_value == CAMERA_EFFECT_OFF)
	{
		switch (saturation)
		{
			...
			case CAMERA_SATURATION_LV4:
				...
				rc = OV7692Core_WritePREG(ov7692_saturation_lv4_tbl);
				break;
			case CAMERA_SATURATION_LV5:
				CDBG("--CAMERA--CAMERA_SATURATION_LV5\n");
				rc = OV7692Core_WritePREG(ov7692_saturation_default_lv5_tbl);
				break;
			...
			case CAMERA_SATURATION_LV9:
				...
				rc = OV7692Core_WritePREG(ov7692_saturation_lv9_tbl);
				break;
			case CAMERA_SATURATION_LV10:
				CDBG("--CAMERA--CAMERA_SATURATION_LV10\n");
				rc = OV7692Core_WritePREG(ov7692_saturation_lv10_tbl);
				break;
			default:
...
static long ov7692_set_effect(int mode, int effect)
{
	...
	ov7692_i2c_write_b_sensor(0x38, 0x50);
	msleep(33);
	effect_value = effect;
	switch (effect)
	{
		case CAMERA_EFFECT_OFF: {
			...
			ov7692_i2c_write_b_sensor(0xda, SAT_U); //for recover saturation level when change special effect
			ov7692_i2c_write_b_sensor(0xdb, SAT_V); //for recover saturation level when change special effect
			break;
		}
		...
		case CAMERA_EFFECT_SOLARIZE: {
			CDBG("--CAMERA-- %s ...CAMERA_EFFECT_NEGATIVE(Not Supported)!\n",__func__);
			break;
		}
	...
	ov7692_i2c_write_b_sensor(0x38, 0x10);
	//Refresh Sequencer /
	CDBG("--CAMERA-- %s ...(End)\n",__func__);
	return rc;
}
...
static int ov7692_set_contrast(int contrast)
{
	...
	if (effect_value == CAMERA_EFFECT_OFF)
	{
		...
		case CAMERA_CONTRAST_LV4:
			...
			rc = OV7692Core_WritePREG(ov7692_contrast_lv4_tbl);
			break;
		case CAMERA_CONTRAST_LV5:
			CDBG("--CAMERA--CAMERA_CONTRAST_LV5\n");
			rc = OV7692Core_WritePREG(ov7692_contrast_default_lv5_tbl);
			break;
        ...
        case CAMERA_CONTRAST_LV9:
            ...
            rc = OV7692Core_WritePREG(ov7692_contrast_lv9_tbl);
            break;
        case CAMERA_CONTRAST_LV10:
            CDBG("--CAMERA--CAMERA_CONTRAST_LV10\n");
            rc = OV7692Core_WritePREG(ov7692_contrast_lv10_tbl);
            break;
		default:
	...
	if (effect_value == CAMERA_EFFECT_OFF)
	{
		switch(sharpness)
		{
			...
			case CAMERA_SHARPNESS_LV2:
				...
				rc = OV7692Core_WritePREG(ov7692_sharpness_default_lv2_tbl);
				break;
...
static int ov7692_set_wb_oem(uint8_t param)
{
    ...
    {
		case CAMERA_WB_AUTO:
			CDBG("--CAMERA-- CAMERA_WB_AUTO %d\n", param);
			rc = OV7692Core_WritePREG(ov7692_wb_auto);
			break;
		case CAMERA_WB_INCANDESCENT:
			CDBG("--CAMERA-- CAMERA_WB_INCANDESCENT %d\n", param);
			rc = OV7692Core_WritePREG(ov7692_wb_incandescent);
			break;
		case CAMERA_WB_FLUORESCENT:
			CDBG("--CAMERA-- CAMERA_WB_FLUORESCENT %d\n", param);
			rc = OV7692Core_WritePREG(ov7692_wb_fluorescent);
			break;
		case CAMERA_WB_DAYLIGHT:
			CDBG("--CAMERA-- CAMERA_WB_DAYLIGHT %d\n", param);
			rc = OV7692Core_WritePREG(ov7692_wb_daylight);
			break;
		case CAMERA_WB_CLOUDY_DAYLIGHT:
			CDBG("--CAMERA-- CAMERA_WB_CLOUDY_DAYLIGHT %d\n", param);
...
int ov7692_sensor_config(void __user *argp)
{
    ...
    switch (cdata.cfgtype)
    {
		...
		case CFG_SET_TOUCHAEC:
		    ...
		    //ov7692_set_touchaec(cdata.cfg.aec_cord.x, cdata.cfg.aec_cord.y);
		    rc = 0 ;
		    break;
