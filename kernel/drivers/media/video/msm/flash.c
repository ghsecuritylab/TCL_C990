...
static int set_led_flashlight(struct msm_camera_sensor_flash_led *led,
	enum led_brightness brightness)
{
	struct led_classdev *led_cdev;

	CDBG("led_name = %s, brightness = %d\n", led->led_name, brightness);
	
	down_read(&leds_list_lock);
	list_for_each_entry(led_cdev, &leds_list, node) {
		if (!strncmp(led_cdev->name, led->led_name, led->led_name_len)) {
			break;
		}
	}
...
int msm_camera_flash_led(struct msm_camera_sensor_flash_led *led, unsigned led_state)
{
	int rc = 0;

	switch (led_state) {
	case MSM_CAMERA_LED_OFF:
		rc = set_led_flashlight(led, LED_OFF);
		break;

	case MSM_CAMERA_LED_LOW:
		rc = set_led_flashlight(led, LED_HALF);
		break;

	case MSM_CAMERA_LED_HIGH:
		rc = set_led_flashlight(led, LED_FULL);
		break;
...
int msm_camera_flash_led_ext(
	struct msm_camera_sensor_flash_led *led,
	unsigned led_state)
{
	int rc = 0;
	...
	switch (led_state) {
	case MSM_CAMERA_LED_OFF:
		gpio_set_value_cansleep(led->led_en, 0);     
		gpio_set_value_cansleep(led->led_mode, 0);
		break;

	case MSM_CAMERA_LED_LOW:
		gpio_set_value_cansleep(led->led_en, 1);     
		gpio_set_value_cansleep(led->led_mode, 0);  
		break;

	case MSM_CAMERA_LED_HIGH:
		gpio_set_value_cansleep(led->led_en, 1);     
		gpio_set_value_cansleep(led->led_mode, 1);
		break;
	...
	struct msm_camera_sensor_flash_data *fdata, unsigned led_state)
{
	...
	case MSM_CAMERA_FLASH_SRC_LED:
		rc = msm_camera_flash_led(&fdata->flash_src->_fsrc.led_src, led_state);
		break;

	case MSM_CAMERA_FLASH_SRC_LED_EXT:
		rc = msm_camera_flash_led_ext(&fdata->flash_src->_fsrc.led_src, led_state);
		break;
