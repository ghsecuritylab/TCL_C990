...
/* CABL flag, bypass all other clients backlight control when CABL enabled */
static int cabl_enable = 0;
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (count == size) {
		...
		if (!strncmp(led_cdev->name, "lcd-backlight", 13)) {
			if (!cabl_enable || !strncmp(current->comm, "mm-abl-daemon", 13)) {
...
static ssize_t led_cabl_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", cabl_enable);
}

static ssize_t led_cabl_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	ssize_t ret = -EINVAL;
	char *after;
...
static DEVICE_ATTR(cabl, 0644, led_cabl_show, led_cabl_store);
...
int led_classdev_register(struct device *parent, struct led_classdev *led_cdev)
{
	...
	/* add cabl sys interface for lcd-backlight */
	if(!(strncmp(led_cdev->name, "lcd-backlight", 13))) {
		if ((ret = device_create_file(led_cdev->dev, &dev_attr_cabl))) {
