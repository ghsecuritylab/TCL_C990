...
#define LED_FLASH_EN 96
#define LED_FLASH_MODE 107
...
struct tricolor_led_data {
	...
	spinlock_t led_lock;
	int led_data;
	struct led_classdev leds;	/* blue, green, red, flashlight */
};
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	tricolor_led = container_of(led_cdev, struct tricolor_led_data, leds);
	if(!tricolor_led)
		printk(KERN_ERR "%s tricolor_led is NULL ",__func__);
	ret = sprintf(buf, "%u\n", tricolor_led->led_data);
	return ret;
}
...
#else
       //qrd  define 3 type brightness level in /include/linux/leds.h
       //but it's hardware only has on/off led function.
       //Otherwise our es02 evb has 3 level led control.

	if(LED_OFF == brightness){
		gpio_set_value(LED_FLASH_EN, 0);
		gpio_set_value(LED_FLASH_MODE, 0);
	}
	else if(LED_HALF == brightness){
		gpio_set_value(LED_FLASH_EN, 1);
		gpio_set_value(LED_FLASH_MODE, 0);
	}	
	else if(LED_FULL == brightness){

		gpio_set_value(LED_FLASH_EN, 1);
		gpio_set_value(LED_FLASH_MODE, 1);
	}
#endif
}

// We did not need tricolor,only need flash led. so we over write it
#if 0
...
static int tricolor_led_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct tricolor_led_data *tricolor_led;
	printk(KERN_ERR "flashlight: probe init \n");
	tricolor_led = kzalloc(sizeof(struct tricolor_led_data), GFP_KERNEL);
	if (tricolor_led == NULL) {
		printk(KERN_ERR "flash_led_probe: no memory for device\n");
		ret = -ENOMEM;
		goto err;
	}
	memset(tricolor_led, 0, sizeof(struct tricolor_led_data));
	...
	ret = IS_ERR(tricolor_led->rpc_client);
	if (ret) {
		printk(KERN_ERR "[flash-led] cannot initialize rpc_client!\n");
		tricolor_led->rpc_client = NULL;
		goto err_init_rpc_client;
	}

	
	tricolor_led->leds.name = "flashlight";
	tricolor_led->leds.brightness_set = led_brightness_set_flash;

		/* flashlight */
	ret = led_classdev_register(&pdev->dev, &tricolor_led->leds);
	if (ret) {
		printk(KERN_ERR
		       "flash_led: led_classdev_register failed\n");
		goto err_led_classdev_register_failed;
	}

	ret = device_create_file(tricolor_led->leds.dev, &dev_attr_blink);
	if (ret) {
		printk(KERN_ERR
		       "flash_led: device_create_file failed\n");
		goto err_out_attr_blink;
		}
	dev_set_drvdata(&pdev->dev, tricolor_led);
	return 0;
...
err_out_attr_blink:
                device_remove_file(tricolor_led->leds.dev, &dev_attr_blink);

err_led_classdev_register_failed:
		led_classdev_unregister(&tricolor_led->leds);

err_init_rpc_client:
	/* If above errors occurred, close pdata->rpc_client */
	if (tricolor_led->rpc_client) {
		oem_rapi_client_close();
		printk(KERN_ERR "flash-led: oem_rapi_client_close\n");
	}
	kfree(tricolor_led);
err:
	return ret;
...
{
	struct tricolor_led_data *tricolor_led;
	printk(KERN_ERR "flash_led_remove: remove\n");
