...
static int smd_tty_dummy_probe(struct platform_device *pdev)
{
	...
	else if (!strncmp(pdev->name, smd_ch_name[8],
				strnlen(smd_ch_name[8], SMD_MAX_CH_NAME_LEN)))
		complete_all(&smd_tty[8].ch_allocated);
...
static int __init smd_tty_init(void)
{
	...
	tty_register_device(smd_tty_driver, 8, 0);
	...
	init_completion(&smd_tty[8].ch_allocated);
	...
	if (ret)
		goto unreg3;
	smd_tty[8].driver.probe = smd_tty_dummy_probe;
	smd_tty[8].driver.driver.name = smd_ch_name[8];
	smd_tty[8].driver.driver.owner = THIS_MODULE;
	spin_lock_init(&smd_tty[8].reset_lock);
	ret = platform_driver_register(&smd_tty[8].driver);
	if (ret)
		goto unreg7;
...
unreg8:
	platform_driver_unregister(&smd_tty[8].driver);
unreg7:
