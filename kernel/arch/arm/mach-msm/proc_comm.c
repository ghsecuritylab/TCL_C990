...
int msm_read_nv(unsigned int nv_item, void *buf)
{
	...
	switch (nv_item)
	{
	/*NV4678 is used to store WLAN_MAC_ADDR*/
	case 4678:
		for(i = 0; i < 6; i++ )
		{
			if(i < 4)
...
/*
	...
	App ARM calls this API to read NV item from Modem ARM
*/
#define PROC_OEM_BASE      0x10000000
#define PROC_OEM_NV_READ  (PROC_OEM_BASE+1)
int msm_oem_read_nv(unsigned int nv_item, void *buf)
{
	int ret = -1;
	uint32_t data1 = nv_item;
	uint32_t data2 ;
	...
	ret = msm_proc_comm(PROC_OEM_NV_READ, &data1, &data2);
	if (ret){
		printk(KERN_ERR "%s:nv item %d READ ERR\n",__func__,nv_item);
		return ret;
	}
	...
	case 7222:
		*dest = data1;
		break;
	case 7234: //CDROM enable
		*dest = data1;
		break;
	case 7235 : //carrier code
		*dest = data1;
		break;
	case 7236 : //factory mode
		*dest = data1;
		break;
	default:
		printk(KERN_ERR "%s:nv item %d is not supported now\n",__func__,nv_item);
...
int read_nv(unsigned int nv_item, void *buf)
{
	...
	switch (nv_item)
	{
		...
		//	printk("buf[0...6] 0x%x 0x%x\n",(unsigned char)*buf,(unsigned char)*(buf+1));
			break;
#endif
		default:
			printk(KERN_ERR "%s:nv item %d is not supported now\n",__func__,nv_item);
