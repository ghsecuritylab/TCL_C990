...
void l2cap_do_send(struct sock *sk, struct sk_buff *skb)
{
	...
	BT_DBG("sk %p, skb %p len %d priority %u", sk, skb, skb->len, skb->priority);
...
struct sk_buff *l2cap_create_connless_pdu(struct sock *sk, struct msghdr *msg, size_t len, u32 priority)
{
	struct l2cap_conn *conn = l2cap_pi(sk)->conn;
	struct sk_buff *skb;
...
struct sk_buff *l2cap_create_basic_pdu(struct sock *sk, struct msghdr *msg, size_t len, u32 priority)
{
	struct l2cap_conn *conn = l2cap_pi(sk)->conn;
	struct sk_buff *skb;
